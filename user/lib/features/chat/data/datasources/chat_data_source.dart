...
class ChatDataSource {
  ...
  Future<ResponseWrapper<DataMessage>> addMessage(AddMessageParams params) {
    return throwAppException(() async {
      final response = await clientApi.request(RequestConfig(
          endpoint:
              '${EndPoints.chats.chat}/${params.chatId}${EndPoints.chats.messages}',
          ...
          data: params.map));
      return ResponseWrapper.fromJson(response.data, (json) {
        final result = DataMessage.fromJson(response.data);
        return result;
      });
    });
  }
  ...
  Future<bool> deleteMessage(DeleteMessageParams params) {
    return throwAppException(() async {
      await clientApi.request(RequestConfig(
          endpoint:
              '${EndPoints.chats.chat}/${params.chatId}${EndPoints.chats.messages}/${params.messageId}',
  ...
  Future<bool> deleteChat(String id) {
    return throwAppException(() async {
      await clientApi.request(RequestConfig(
          endpoint: '${EndPoints.chats.chat}/$id',
          clientMethod: ClientMethod.delete));
